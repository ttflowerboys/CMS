...
CREATE TABLE IF NOT EXISTS `xiao_category` (
  ...
  `mark` smallint(3) NOT NULL,
  PRIMARY KEY (`catid`),
  KEY `listorder` (`listorder`,`catid`,`siteid`)
) ENGINE=MyISAM  DEFAULT CHARSET=utf8;
...
DROP TABLE IF EXISTS `xiao_site`;
CREATE TABLE IF NOT EXISTS `xiao_site` (
...
INSERT INTO `xiao_site` (`siteid`, `site_name`, `site_title`, `site_theme`, `site_keywords`, `site_description`, `site_domain`, `site_language`, `language`) VALUES
(1, 'XiaoCms企业建站版', 'XiaoCms中文版演示站', 'default', 'xiaocms', 'XiaoCms中文版演示站', 'www.domain.com', 'cn', '中文版');
