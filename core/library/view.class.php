...
class view
{
    ...
    protected function _category($param)
    {
        ...
        if (is_array($param)) {
            foreach ($param as $key => $val) {
                if (in_array($key, array('catid', 'siteid', 'typeid', 'modelid', 'parentid', 'num', 'ismenu'))) {
                    $system[$key] = $val;
                }
            }
        }
        ...
        foreach ($this->cats as $catid => $cat) {
            ...
            if ($system['siteid']) {
                if ($cat['siteid'] != $system['siteid']) continue;
            } else {
                if ($cat['siteid'] != SITEID) continue;
            }
            $data[$catid] = $cat;
            $i++;
        }
    ...
    public function get_category_url($data, $page = 0)
    {
        ...
        return 'http://'.$site[$data['siteid']]['site_domain'].'/' . $url;
    }
    ...
    public function get_show_url($data, $page = 0)
    {
        ...
        if (!empty($this->site_config['diy_url']) && $this->site_config['show_url']) {
            ...
            $data['siteid'] = $this->cats[$data['catid']]['siteid'];
            $data['page'] = '[page]';
            if (!empty($page))
        ...
        return 'http://'.$site[$this->cats[$data['catid']]['siteid']]['site_domain'].'/' . $url;
    }
