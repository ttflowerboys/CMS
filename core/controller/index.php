...
class index extends Base {
	...
	if($this->get('catdir') || $this->get('catid')){
	    ...
	    if (empty($category)) {
			...
			$this->show_message(L('9'));
		}
   	    $category['page']   = (int)$this->get('page') ? (int)$this->get('page') : 1;
		if($category['islook'] && !$this->member_info) $this->show_message(L('10'));
	    ...
	    if (empty($content)) {
		    header('HTTP/1.1 404 Not Found');
		    $this->show_message(L('11'));
		}
	    if (empty($content['status'])) $this->show_message(L('12'));
	...
	public function searchAction() {
	    ...
	    $this->view->assign(array(
	        ...
	        'data' => $data,
			'num' => $total,
			'site_title'  => $kw . ' - ' . $this->site_config['site_name'],
			'site_keywords'    => $kw, 
			'site_description' => $kw,
	    ));
	...
	public function formAction() {
		...
		!empty($form_model)  or $this->show_message(L('14'));
		if (!empty($form_model['joinid'])) {
	    	!empty($cid) or $this->show_message(L('15'));
	    	$this->db->setTableName('content')->getOne(array('id=?', 'modelid=?'), array($cid, $form_model['joinid']),'id')  or $this->show_message(L('16'));
		}
	    if ($this->post('submit')) {
    		$gobackurl = $this->post('gobackurl');
    		if (!empty($form_model['setting']['form']['code']) && !$this->checkCode($this->post('code'))) $this->show_message(L('17'),2,1);
			if (!empty($form_model['setting']['form']['post']) && !$this->member_info) $this->show_message(L('18'),2,1);
			if (!empty($form_model['setting']['form']['time'])){
		        ...
		        if (time() - $ipdata['time'] < $time) $this->show_message(L('19'). $form_model['setting']['form']['time'] .L('20'),2,1);
			}
			if (!empty($form_model['setting']['form']['num']) && !empty($form_model['setting']['form']['post']) && $this->member_info ) {
				...
				if (!empty($form_model['joinid'])) $this->db->where('cid=?', $cid);
		        if ($this->db->getOne('','','id')) $this->show_message(L('21'),2,1);
			}
			$data = $this->post('data');
			...
			if ($this->db->setTableName($form_model['tablename'])->insert($data,true)) {
			    $this->show_message($data['status'] ? L('22') : L('24'), 1, $gobackurl);
			} else {
			    $this->show_message(L('23'),2,1);
			}
		}
	    $this->view->assign(array(
