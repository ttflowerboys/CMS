...
#root_menu_name{display: block;}
#root_menu_name ul{display: none;background: #fff url(./img/bg_y.gif) repeat-y <?php echo $left_width-6 ;?>px 0;border-bottom: 2px solid #D0E6EC;position: absolute;left: 0;width: 100%;}
#root_menu_name li{border-bottom: 1px solid #D0E6EC; padding-left: 20px;}
#root_menu_name a{display: block;color: #f60;}
#root_menu_name li:hover{background: #D0E6EC;}
</style>
</head>
<body scroll="no">
<!--头部开始-->
  ...
  <div id="menu_position">
    <ul id="menu">
        <li id="_MP104" ><a href="javascript:_MP(104,'<?php echo url('index/my') ;?>');">设置</a>
          ...
          <li id="_MP1043" ><a href="javascript:_MP(1043,'<?php echo url('site') ;?>');" >站点管理</a></li>
...
<div id="main">
  ...
  <div id="left">
    <h2>
      ...
      <label id='root_menu_name'><a href='<?php echo url('content'); ?>' target='right'><?php echo $sitename;?></a>
        <ul id="site">
          <?php foreach ($site as $value):?>
            <li><a href="?site=<?php echo $value['siteid'];?>"><?php echo $value['language'];?></a></li>
          <?php endforeach; ?>
        </ul>
      </label>      
    </h2>
    <iframe name="leftMain" id="leftMain" src="<?php echo url('index/tree',array('site'=>$siteid)); ?>" frameborder="false" scrolling="auto" style="border:none" width="100%" height="600" allowtransparency="true"></iframe>
  </div>
  <!--左侧结束-->
  <!--右侧开始-->
  <div id="right">
    <div id="home">
    	<div id="shortcut">
      <?php foreach ($site as $value):?>
        <span><a href="http://<?php echo $value['site_domain'];?>"  target="_blank" ><?php echo $value['language'];?>首页</a></span>
      <?php endforeach; ?>
